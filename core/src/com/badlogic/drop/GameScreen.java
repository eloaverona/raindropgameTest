...
public class GameScreen implements Screen {
	...
	//keeps track of how many drops were gathered
	int dropsGathered;
	...
	public GameScreen (final Drop game) {
		...
		rainMusic.setLooping(true); //music plays on repeat
	...
	public void render (float delta) {
		...
		game.batch.begin(); //tells SriteBatch to start new Batch

		game.batch.draw(bucketImage, bucket.x, bucket.y); //renders bucket in the screen

		for(Rectangle raindrop: raindrops){
			game.batch.draw(dropImage, raindrop.x, raindrop.y); //renders drops in the screen
		}

		game.font.draw(game.batch, "Drops Collected: " + dropsGathered, 0, 480);

		game.batch.end(); //all draw commands between batch.begin() and batch.end will be made at once. It speeds up rendering
